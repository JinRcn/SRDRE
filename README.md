...
- **Step 3**:  Assign spatial annotations to RNA editing sites and calculate RNA editing index
  ...
  **Output**  
  ...
  - A summary table presenting the REI (RNA editing index) for each region and cell type (`.REI.tsv`).
...
The RNA editing index (REI), which reflects global RNA editing activity, is defined as the ratio of edited bases (identified as G) to the sum of edited and unedited bases (A + G).
...
If you have any comments or suggestions, please contact Dr. Huang (jinrcn@foxmail.com or huangjinrong@genomics.cn).
